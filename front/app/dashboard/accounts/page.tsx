...
import { PlusCircle, Edit, Trash2, Instagram, BarChart3, WifiOff, Wifi, Loader2 } from "lucide-react"
...
export default function AccountsPage() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      setError(`Erreur lors du chargement des comptes. URL: ${process.env.NEXT_PUBLIC_BACK_URL || 'http://localhost:3001/api'}`)
    } finally {
      setLoading(false)
    }
  }
            ...
            <Card key={account.id}>
              <CardHeader className="flex flex-row items-center gap-4">
                <div className="relative h-12 w-12 rounded-full overflow-hidden bg-gray-200">
                  {account.mainImage ? (
                    <Image
                      src={`${process.env.NEXT_PUBLIC_BACK_URL?.replace('/api', '') || 'http://localhost:3001'}/api/images/${account.mainImage.id}`}
                      alt={account.name}
                      fill
